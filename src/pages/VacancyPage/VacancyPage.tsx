...
export class VacancyPage extends React.PureComponent<VacancyPageProps> {
  render() {
    const date = moment(this.props.vacancy.publishedDate).format(
      "DD MMMM YYYY",
    )
    ...
    return (
      <ColumnsPageContainer>
        <AsideLeftContainer>
          <Link to={this.props.company.link || "#"} className={CSS.companyLink}>
            ...
            <div className={CSS.companyVacancies}>
              ...
              {__t("activeVacancies", {
                count: this.props.company.vacancyPublished.length,
              })}
            </div>
          </Link>
          <div className={CSS.companyCity}>{this.props.company.city.name}</div>
          <BlueButton className={CSS.applyButton}>
            {__t("vacancyPage.recall")}
          </BlueButton>
          <GreenButton className={CSS.showPhoneButton}>
            {__t("vacancyPage.showPhone")}
          </GreenButton>
          <time className={CSS.dateAndNumber}>
            {date} №{this.props.vacancy.number}
          </time>
        </AsideLeftContainer>
        ...
        <MainContainer>
          <header
            className={classNames(CSS.mainHeader, {
              [CSS.mainHeaderHot]: this.props.vacancy.isHot,
            })}>
            <h1 className={CSS.vacancyTitle}>{this.props.vacancy.title}</h1>
            <div className={CSS.salary}>{this.props.vacancy.salaryText}</div>

            {this.props.vacancy.isHot && (
              <div
                className={classNames(CSS.headerSpecial, CSS.headerSpecialHot)}>
                <span className={CSS.headerSpecialContent}>
                  {__t("vacancyPage.hot")}
                </span>
              </div>
            )}

            {this.props.vacancy.isRecommended && (
              <div
                className={classNames(
                  CSS.headerSpecial,
                  CSS.headerSpecialRecommended,
                )}>
                <span className={CSS.headerSpecialContent}>
            ...
            {this.props.vacancy.isActive && (
              <div
                className={classNames(
                  CSS.headerSpecial,
                  CSS.headerSpecialActive,
                )}>
                <span className={CSS.headerSpecialContent}>
              ...
              {this.props.vacancy.params.map((param: string[], index) => {
                return (
                  <li className={CSS.descriptionItem} key={index}>
                    <span className={CSS.descriptionItemTitle}>
                      {param[0]}:
                    </span>
                    {param[1]}
                  </li>
          ...
          {!!this.props.company.photoSet.length && (
        ...
        <AsideRightContainer>
          <AsideVacancyList
            vacancies={[vacancyMock, vacancyMock, vacancyMock]}
          />
          {this.props.banner && (
            <VerticalBanner
