...
export const CatalogItemVacancy: React.SFC<CatalogItemVacancyProps>  = (props) => {
    ...
    const isActually = props.vacancy.isActually
    const isClosed = props.vacancy.isClosed
    const needRenderParams = !isHot && !isActiveSearch && !isActually && !isClosed

    const address = props.vacancy.addressesList && props.vacancy.addressesList.find((a) => !!a.station)

    return (
        <div className={classNames(CSS.item, props.className, {[CSS.shortInfo]: props.shortInfo})}>
            ...
            {!props.shortInfo && <div className={CSS.itemRightContent}>
                {isHot && <span className={CSS.isHot}/>}
                {isActually && <span className={CSS.isActually}/>}
                {isClosed && <span className={CSS.isClosed}/>}
